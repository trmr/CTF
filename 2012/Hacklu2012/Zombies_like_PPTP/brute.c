...
void bruteforce_set_charset(char * _charset, size_t _charset_len) {
    ...
    if (charset_len > sizeof(charset)) {
        printf("ERROR: too long charset (%d bytes)\n", charset_len);
        exit(100);
    }
...
void bruteforce(size_t _nthreads, check_func_t _check_func) {
    ...
    if (debug) printf("Starting bruteforce with %d threads...\n", nthreads);
    if (debug) printf("Charset: '%s' (%d bytes)\n", charset, charset_len);
    if (debug) printf("Key: up to %d bytes\n", max_keylen);
    if (debug) printf("\n");
    ...
    while (1) {
        ...
        while (add) {
            ...
            if (nesting >= key_len) { // key growing
                key_len = nesting + 1;
                if (nesting == 0)
                    pos[nesting] = thread_index;
                else
                    pos[nesting] = 0;
                if (debug) printf("Raised key length up to %d...\n", key_len);
            }
            else { // increasing char
                if (nesting == 0)
                    pos[nesting] += nthreads;
                else
                    pos[nesting]++;
            }
