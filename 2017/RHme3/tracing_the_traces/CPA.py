...
NUM_TRACES = len(input['samples'])
NUM_POINTS = len(input['samples'][0])

tr_list = [input['samples'][x] for x in range(NUM_TRACES)]
pt_list =  [input['inout'][x][0:16] for x in range(NUM_TRACES)]
ct_list =  [input['inout'][x][16:32] for x in range(NUM_TRACES)]
...
print "[+] given plaintext: {}".format("".join(map(binascii.hexlify, pt_list[0])))
print "[+] given ciphertext: {}".format("".join(map(binascii.hexlify, ct_list[0])))

pt =  "".join(map(chr, pt_list[0]))
ct =  "".join(map(chr, ct_list[0]))

aes = AES.new(strkey)
ct2 = aes.encrypt(pt)

print "[+] made ciphertext: {}".format("".join(map(binascii.hexlify, ct2)))

assert ct2 == ct, 'guess key is wrong'

print "[+] Guess is correct!"

"""
[+] Best key guess: CAFEBABEDEADBEEF0001020304050607
"""