...
def encrypt(pk, m): # pk[0] = p*q, pk[1] = d^-1 mod (p-1)(q-1)
  assert m < pk[0]
  return pow(m, pk[1], pk[0])

def decrypt(pk, sk, c): # sk[0] = d
  return pow(c, sk[0], pk[0])

def test(n, p, q, bits=1024):
  d_ = get_random_prime(int(math.floor(1024 * 0.2))) # get_random_prime(204)
  d = (p-1) * (q-1) - d_ # d = (p-1)(q-1) - rand_prime(204)
